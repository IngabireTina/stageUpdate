...
def homePage(request):
    return render(request, 'registration/present1.html')
...
@method_decorator(login_required, name='dispatch')
class Dashboard22(ListView):
    model = Item
    template_name = 'dashboard2.html'

    def get_context_data(self, *, object_list=None, **kwargs):
        context = super(Dashboard22, self).get_context_data(**kwargs)
...
class Dashboard2(ListView):
    ...
    def get_context_data(self, *, object_List=None, **kwargs):
        ...
        context['computer_lap'] = Stock.objects.filter(category='Computer Laptop').count()
        # context['computer_lap_av'] = Stock.objects.filter(category='Computer Laptop').count()
        context['laptop_given'] = Item.objects.filter(device__category='Computer Laptop').count()
        context['computer_desk'] = Stock.objects.filter(category='Computer Desktop').count()
        context['desktop_given'] = Item.objects.filter(device__category='Computer Desktop').count()
        ...
        context['all_device'] = Stock.objects.filter(availability='Available').count()
        context['all_given'] = Stock.objects.filter(availability='Given').count()
        context['all_givens'] = Item.objects.all().count()

        context['laptop_av'] = Stock.objects.filter(availability='Available').filter(category='Computer Laptop').count()

        context['all_available_device'] = Stock.objects.filter(availability='Available').count()

        context['all_stocks'] = Stock.objects.all()

        """snippets for counting items not working Admin dashboard"""

        context['not_working'] = Stock.objects.filter(item__status='Not Working').count()
        context['under_maintenance'] = Stock.objects.filter(item__status='Undermaintenance').count()
